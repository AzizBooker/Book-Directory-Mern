  ...
  .get((req, res) => {
    ...
    Book.find({author:req.query.author}).then((book) => {
        ...
        if(req.query.mode==="gte"){
            Book.where("rating").gte(req.query.rating).then((book)=>{
                res.json(book)
            })
        }
        if(req.query.mode==="lte"){
            Book.where("rating").lte(req.query.rating).then((book)=>{
                res.json(book)
            })
        }
        if(req.query.mode==="gt"){
            Book.where("rating").gt(req.query.rating).then((book)=>{
                res.json(book)
            })
        }
        if(req.query.mode==="lt"){
            Book.where("rating").lt(req.query.rating).then((book)=>{
                res.json(book)
            })
        }
